...
def evaluate_lenet5(learning_rate=0.1, n_epochs=500, L1_reg=0.00, L2_reg=0.00001,
					dataset='cifar-10-batches-py',
					nkerns=[20, 50], batch_size=100, mode='train', amount='full'): # nkerns=[6, 16], batch_size=100
	...
	if mode == 'test':
		learned_params = unpickle('params/convolutional_mlp.pkl')
	...
	if mode == 'test':
		...
		train_error_model = theano.function([index], layer3.errors(y),
				givens={
	...
	while (epoch < n_epochs) and (not done_looping):
		...
		for minibatch_index in xrange(n_train_batches):
			...
			if (iter + 1) % validation_frequency == 0:
				...
				training_losses = [train_error_model(i) for i
				...
				if mode == 'train':
					...
					save_parameters(get_params(), 'convolutional_mlp')
	...
	if mode == 'test':
		...
		writer = csv.writer(file('result/convolutional_mlp.csv', 'w'))
		row = 1
